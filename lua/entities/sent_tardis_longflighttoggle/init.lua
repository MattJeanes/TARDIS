...
function ENT:Initialize()
	self:SetModel( "models/drmatt/legtar/longflighttoggle.mdl" )
...
function ENT:Toggle( bEnable, ply )
	...
	if IsValid(self.tardis) then
		local success=self.tardis:ToggleLongFlight()
		if success then
			if self.tardis.longflight then
				ply:ChatPrint("TARDIS long-flight enabled.")
			elseif not self.tardis.longflight then
