...
function ENT:UpdateScreenInfo()
	local id
	if IsValid(self.interior) and self.interior.interior and self.interior.interior.ID then
		id=self.interior.interior.ID
	else
		id="default"
	end
...
for k,v in pairs(ents.FindByClass("gmod_interior")) do
	v:UpdateScreenInfo(v)
end
...
function ENT:ScreenActive(name)
	local t={}
	if self.interior.screens3D then
		for k,v in pairs(self.interior.screens3D) do
...
function ENT:LoadScreenUI(screen)
	...
	if IsValid(self.interior) then
		for k,v in pairs(self.interior:GetScreens()) do
			t[k]=v
		end
	end
