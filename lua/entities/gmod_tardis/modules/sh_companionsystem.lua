        ...
        addMenuBtn.DoClick = function(me)
            ...
            addPlayerBtn.DoClick = function(me)
                ...
                if onlinePlayersList.Players[ID] then
                    ...
                    local ply = onlinePlayersList.Players[ID]
                    
                    -- Add to companions list
                    lst:AddPlayer({Name=ply:Name(), SteamID=ply:SteamID()})
                    hook.Run("TARDIS-UpdateCustomOption", lst.Players)
...
ENT:AddHook("CanUsePart","companion_system_security",function(self, part, ply)
    local companionList = TARDIS:GetSetting("companions-list", self:GetCreator())
    PrintTable(companionList)
end)
