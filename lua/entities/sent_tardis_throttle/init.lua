...
function ENT:Initialize()
	self:SetModel( "models/drmatt/legtar/throttle.mdl" )
...
function ENT:Toggle( bEnable, ply )
	...
	if IsValid(tardis) and IsValid(interior) then
		...
		if self.tardis.moving and self.tardis.invortex and self.tardis.longflight and not self.tardis.reappearing then
	...
	if IsValid(self.tardis) then
		net.Start("TARDISInt-ControlSound")
			...
			net.WriteString("legtar/control_throttle.wav")
		net.Broadcast()
	end
end