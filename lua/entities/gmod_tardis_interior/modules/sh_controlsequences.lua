...
ENT:AddHook("CanStartControlSequence", "conditions", function(self,id)
    local cseqs = TARDIS:GetControlSequence(self:GetIntMetadata().Sequences)
    local seq = cseqs[id]
...
if SERVER then
    ...
    ENT:AddHook("PartUsed","HandleControlSequence",function(self,part,a)
        local sequences = TARDIS:GetControlSequence(self:GetIntMetadata().Sequences)
        ...
        if active==false and sequences[id] then
            ...
            self:EmitSound(self:GetIntMetadata().Sounds.SequenceOK)
        ...
        elseif active==true then
            ...
            if sequences[curseq].Controls[step] == id then
                ...
                self:EmitSound(self:GetIntMetadata().Sounds.SequenceFail)
