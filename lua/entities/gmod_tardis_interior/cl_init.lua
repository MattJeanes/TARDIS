...
ENT:AddHook("PlayerInitialize", "interior", function(self)
    ...
    if GetConVarNumber("r_rootlod")>0 then
		Derma_Query("The TARDIS Interior might require model detail on high, set now?", "TARDIS Interior", "Yes", function() RunConsoleCommand("r_rootlod", 0) end, "No", function() end)
	end
end)
